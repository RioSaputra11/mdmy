let handler = async(m, { conn }) => {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    ...
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://instagram.com/riosapuuttrraa_?igshid=YmMyMTA2M2Y= `, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fkonn) 
